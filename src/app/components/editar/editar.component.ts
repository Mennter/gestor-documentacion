...
export class EditarComponent implements OnInit, OnDestroy {
  ...
  private createForm() {
    ...
    //   licitacion: new FormControl(this.registro.licitacion),
    //   capitulo: new FormControl(this.registro.capitulo),
    //   titulo: new FormControl(this.registro.titulo, Validators.required),
    //   subtitulo: new FormControl(this.registro.subtitulo , Validators.required),
    //   observaciones: new FormControl(this.registro.observaciones),
    //   palabrasClaves: new FormControl(this.registro.palabrasClaves , Validators.required),
    //   pregunta: new FormControl(this.registro.pregunta),
    //   texto: new FormControl(this.registro.texto , Validators.required),
  ...
  public saveRegistro() {
    ...
    this.registroPostSub = this.registroService.save(registro).subscribe({
      next: () => {
        this.cargando = false;
        this.matSnackBar.open( this.editar ? 'Modificado correctamente' : 'Guardado correctamente', 'Ok', { duration: 5000 });
        if (!this.editar) {
          this.volver();
        }
      },
  ...
  anterior() {
    this.router.navigate([`../editar/${this.listaRegistros[this.indiceRegistro - 1]}`]);
  }
